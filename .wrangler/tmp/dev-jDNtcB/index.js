...
// .wrangler/tmp/bundle-isqwiG/checked-fetch.js
...
async function createOrder(env, body) {
  ...
  try {
    const product = await env.DB.prepare("SELECT title, instant_delivery, normal_delivery_text FROM products WHERE id = ?").bind(Number(body.productId)).first();
    if (product) {
  ...
  console.log("\u{1F4E6} Order created:", orderId, "Delivery:", deliveryMinutes, "minutes");
...
async function createPlanCheckout(env, body, origin) {
  ...
  const priceValue = amount !== null && amount !== void 0 && !isNaN(Number(amount)) && Number(amount) > 0 ? Number(amount) : basePrice;
    ...
    if (eventType === "payment.succeeded") {
      ...
      if (checkoutSessionId && apiKey) {
        ...
        try {
          ...
          if (planId) {
            ...
            console.log("\u{1F5D1}\uFE0F Plan deleted immediately after payment:", planId);
          }
        } catch (e) {
      ...
      if (metadata.product_id) {
        try {
          ...
          console.log("\u{1F4E6} Final delivery time for order:", deliveryTimeMinutes, "minutes");
          const encryptedData = JSON.stringify({
        ...
        if (checkout.plan_id) {
          try {
            ...
            const timeoutPromise = new Promise(
              (_, reject) => setTimeout(() => reject(new Error("API timeout")), 5e3)
            );
            ...
            if (archiveResp && archiveResp.ok) {
              ...
              const deleteTimeoutPromise = new Promise(
                (_, reject) => setTimeout(() => reject(new Error("Delete timeout")), 5e3)
              );
...
async function handleSecureDownload(env, orderId, baseUrl) {
  ...
  const openOnly = lowered.includes("youtube.com") || lowered.includes("youtu.be") || lowered.includes("vimeo.com") || lowered.includes("iframe.mediadelivery.net/embed/") || lowered.includes("video.bunnycdn.com/play/") || lowered.includes("archive.org/details/") && !lowered.includes("/download/");
  if (openOnly) {
...
async function buildSitemapXml(env, request, partIndex = null) {
  ...
  if (s.sitemap_include_pages) {
    ...
    for (const p of defaultPaths) {
      ...
      if (allowIndex && inSitemap) {
        ...
        urls2.push({ loc, lastmod: null, changefreq: rule?.changefreq || "daily", priority: typeof rule?.priority === "number" ? rule.priority : 0.7 });
      }
    }
  ...
  if (partIndex && Number.isFinite(Number(partIndex))) {
    ...
    const slice = urls2.slice(start, start + maxUrls);
    return { status: 200, body: urlsetXml(slice), contentType: "application/xml" };
  }
  if (urls2.length > maxUrls) {
    const parts = Math.ceil(urls2.length / maxUrls);
    return { status: 200, body: sitemapIndexXml(baseUrl, parts), contentType: "application/xml" };
  }
  return { status: 200, body: urlsetXml(urls2), contentType: "application/xml" };
}
  ...
  async scheduled(event, env, ctx2) {
    ...
    try {
      if (env.DB) {
        ...
        const timeoutPromise = new Promise(
          (_, reject) => setTimeout(() => reject(new Error("Cleanup timeout after 30s")), 3e4)
        );
...
// .wrangler/tmp/bundle-isqwiG/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-isqwiG/middleware-loader.entry.ts
