...
// Contains all section templates (hero, features, products, blogs, etc.)

(function(global) {
  'use strict';

  const PageBuilderTemplates = {
    // Section Templates - Basic building blocks
    sections: {
      hero: `
        ...
        <section style="padding:60px 20px;max-width:900px;margin:0 auto;">
          ...
          <div class="faq-container" style="display:flex;flex-direction:column;gap:12px;">
            <div class="faq-item" style="border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;">
              <button class="faq-toggle" onclick="this.parentElement.classList.toggle('open')" style="width:100%;padding:18px 20px;background:#f9fafb;border:none;text-align:left;cursor:pointer;display:flex;justify-content:space-between;align-items:center;font-size:1rem;font-weight:600;color:#1f2937;">
        ...
        content: `
          ...
          <script>
            ...
            document.getElementById('quick-ask-form').addEventListener('submit', async function(e) {
              ...
              try {
                var res = await fetch('/api/forum/submit-question', {
                  method: 'POST',
            ...
            async function loadForumQuestions(page) {
              currentPage = page || 1;
              var container = document.getElementById('forum-questions-container');
              container.innerHTML = '<p style="text-align:center;color:#6b7280;padding:20px;">Loading questions...</p>';
              try {
                var res = await fetch('/api/forum/questions?limit=20&page=' + currentPage);
                var data = await res.json();
                if (data.questions && data.questions.length > 0) {
                  totalPages = data.pagination.totalPages || 1;
                  container.innerHTML = '<h3 style="margin-bottom:20px;color:#1f2937;">Recent Questions</h3>' + data.questions.map(function(q) {
    ...
    // Advanced Templates - Full page layouts with more styling
    advancedTemplates: {
      home: {
        title: 'Home',
        slug: 'home',
    ...
    getHeroSection: function(featureImg) {
      if (featureImg) {
    ...
    // Get all merged templates
    getAllTemplates: function() {
      var merged = {};
      Object.assign(merged, this.templates);
      Object.assign(merged, this.advancedTemplates);
      return merged;
    }
  };

  // Export to global
  global.PageBuilderTemplates = PageBuilderTemplates;
