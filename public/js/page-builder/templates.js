var PageBuilderTemplates = (function() {
  var TEMPLATES = {
    home: {
      title: 'Home',
      slug: 'home',
            ...
            try {
              ...
              if (data.questions && data.questions.length > 0) {
                ...
                container.innerHTML = '<h3 style="margin-bottom:20px;color:#1f2937;">Recent Questions</h3>' + data.questions.map(function(q) {
                  ...
                  return '<a href="' + questionHref + '" style="display:block;background:white;border-radius:12px;padding:20px;margin-bottom:15px;box-shadow:0 2px 8px rgba(0,0,0,0.06);text-decoration:none;transition:transform 0.2s,box-shadow 0.2s;"><h4 style="color:#1f2937;margin-bottom:8px;font-size:1.1rem;">' + PageBuilderUtils.escapeHtml(q.title) + '</h4><p style="color:#6b7280;font-size:0.9rem;line-height:1.5;margin-bottom:10px;">' + PageBuilderUtils.escapeHtml(preview) + '</p><div style="display:flex;gap:15px;font-size:0.85rem;color:#9ca3af;"><span>👤 ' + PageBuilderUtils.escapeHtml(q.name) + '</span><span>💬 ' + (q.reply_count || 0) + ' replies</span></div></a>';
                }).join('');
                renderPagination();
              } else {
  ...
  function loadTemplate(type) {
    ...
    PageBuilderUI.setPageType(template.type);
    
    canvas.innerHTML = '';
    
    if (template.content) {
      var wrapper = PageBuilderUtils.createSectionWrapper();
      wrapper.innerHTML = template.content + PageBuilderUtils.createControls();
      canvas.appendChild(wrapper);
    } else if (template.sections) {
      template.sections.forEach(function(sectionType) {
        PageBuilderSections.addSection(sectionType);
      });
    }
