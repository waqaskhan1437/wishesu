    ...
    // Get hero section with optional feature image
    function getHeroSection() {
      const featureImg = document.getElementById('feature_image_url').value.trim();
      if (featureImg) {
    ...
    Object.assign(TEMPLATES, ADVANCED_TEMPLATES);
    ...
    function handleBuilderClick(event) {
      ...
      if (actionEl) {
        ...
        switch (actionEl.dataset.builderAction) {
          case 'preview-mode':
            setPreviewMode(actionEl.dataset.mode || 'desktop');
            break;
          case 'load-template':
            loadTemplate(actionEl.dataset.template);
            break;
          case 'add-section':
            addSection(actionEl.dataset.section);
            break;
          case 'set-page-type':
            setPageType(actionEl.dataset.pageType);
            break;
          ...
          case 'save-code':
            ...
            break;
          case 'move-up':
            if (wrapper) moveSection(wrapper, -1);
            break;
          case 'move-down':
            if (wrapper) moveSection(wrapper, 1);
            break;
          case 'edit-code':
            if (wrapper) editCode(wrapper);
            break;
          case 'duplicate-section':
            if (wrapper) duplicateSection(wrapper);
            break;
          case 'delete-section':
            if (wrapper) deleteSection(wrapper);
            break;
        }
        return;
      }
    ...
    function renderFeaturePreview(url) {
    ...
    function addSection(type) {
      const canvas = document.getElementById('builder-canvas');
      const wrapper = createSectionWrapper();
      const sectionHtml = type === 'hero' ? getHeroSection() : SECTIONS[type];
    ...
    function generateOutputHtml() {
      // Group each section-wrapper into a single container so that custom code
      // containing multiple top-level elements remains intact on reload. Without
      // grouping, each child element becomes its own section on load, which
      // causes the layout to break. Each group will be wrapped in a
      // <div data-section="true"> for later identification during load.
      const parts = [];
      document.querySelectorAll('#builder-canvas .section-wrapper').forEach(wrapper => {
        // Clone the wrapper to safely manipulate its children
        const cloneWrapper = wrapper.cloneNode(true);
        // Remove control buttons from the clone
        cloneWrapper.querySelectorAll('.section-controls').forEach(el => el.remove());
        // Strip builder-only attributes and UI elements
        ...
        // Clean up widget container IDs and clear preview content
        cloneWrapper.querySelectorAll('.product-widget-container, .blog-widget-container, .reviews-widget-container').forEach(el => {
          el.removeAttribute('id');
          el.innerHTML = '';
        });
        // Wrap the entire clone contents in a container element with a marker
        const sectionHTML = `<div data-section="true">${cloneWrapper.innerHTML}</div>`;
