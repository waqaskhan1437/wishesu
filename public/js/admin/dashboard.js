(function() {
  ...
  panel.innerHTML = `
    <div style="display:flex; gap:16px; height:calc(100vh - 170px); min-height:560px;">
      ...
      <div style="flex:1; background:#fff; border:1px solid #d1d5db; border-radius:10px; overflow:hidden; display:flex; flex-direction:column;">
        <div id="chats-detail-header" style="padding:12px 14px; border-bottom:1px solid #e5e7eb; display:flex; align-items:center; justify-content:space-between; gap:12px;">
          <div>
          ...
          <div style="display:flex; gap:8px; align-items:center;">
            <button id="chats-block-btn" class="btn btn-secondary" style="padding:8px 10px; border-radius:8px; display:none;">🚫 Block User</button>
            <button id="chats-delete-btn" class="btn btn-danger" style="padding:8px 10px; border-radius:8px; display:none; background:#dc2626; color:white; border:none;">🗑️ Delete Chat</button>
          </div>
        </div>
  ...
  function updateBlockedUI(isBlocked) {
    ...
    // Admin can still message, but we visually indicate state
  }
  ...
  function renderSessionsList() {
    ...
    for (const s of filtered) {
      ...
      // Row styling similar to Orders listing tone
      row.style.cssText = `
        padding:12px 14px;
        border-bottom:1px solid #f3f4f6;
        cursor:pointer;
        background:${isActive ? '#111827' : (isBlocked ? '#fef2f2' : '#fff')};
