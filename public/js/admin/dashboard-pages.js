...
(function(AD) {
  ...
  AD.loadPages = async function(panel) {
    ...
    if (createBtn) {
      createBtn.addEventListener('click', () => {
        window.location.href = '/admin/page-builder-v2.html';
      });
    }
    ...
    try {
      ...
      if (!data.success || !data.pages || data.pages.length === 0) {
        container.innerHTML = `<div style="text-align:center;padding:40px 20px;color:#6b7280;">
          ...
          <button class="btn btn-primary" onclick="window.location.href='/admin/page-builder-v2.html'">Create Page</button>
        </div>`;
        return;
      }
  ...
  window.editAdminPage = function(slug) {
    if (!slug) return;
    window.location.href = '/admin/page-builder-v2.html?edit=' + encodeURIComponent(slug);
  };
