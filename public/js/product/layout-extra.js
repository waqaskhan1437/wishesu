...
;(function(){
  ...
  // Use shared URL helpers from product/url-helpers.js
  const { splitUrlSuffix, optimizeGoogleusercontentUrl } = window._productUrlHelpers || {};
  ...
  function renderProductDescription(wrapper, product) {
    ...
    setTimeout(() => {
      ...
      if ((product.reviews && product.reviews.length > 0) || typeof window.ReviewsWidget !== 'undefined') {
        if (product.reviews && product.reviews.length > 0) {
          ...
          // Reviews widget loads after the product scripts on this page.
          // If SSR cards are absent and the widget is not ready yet, leave the
          // current markup intact instead of replacing it with an empty state.
