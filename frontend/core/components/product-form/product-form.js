...
import { createAddonBuilder } from '../../addons/builder/builder.js';
...
export function ProductForm({ onSaved }) {
  ...
  form.innerHTML = `
    ...
    <div class="tab-panel active" data-panel="basic">
      ...
      <label>Normal Price (USD)</label>
      <input name="normal_price" type="number" placeholder="2400" />
      <label>Sale Price (USD)</label>
      <input name="sale_price" type="number" placeholder="2000" />
  ...
  const setValues = (product) => {
    ...
    form.querySelector('[name="normal_price"]').value = product.normal_price ?? product.price ?? 0;
    ...
    addonBuilder.set(product.addons || []);
  };
  form.addEventListener('submit', async (e) => {
    ...
    const payload = {
      ...
      sale_price: data.sale_price === '' ? null : Number(data.sale_price || 0),
      price: Number(data.normal_price || 0),
      status: data.status,
