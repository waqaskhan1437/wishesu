...
<script>
const S={tab:0,completed:new Set(),galleries:[],videos:[],addons:[],uploadingCount:0};
...
function switchTab(i){
  ...
  document.getElementById('prev').disabled=i===0||S.uploadingCount>0;
  document.getElementById('next').disabled=S.uploadingCount>0;
    ...
    return new Promise((resolve,reject)=>{
      ...
      xhr.onload=()=>{
        ...
        if(xhr.status===201){
          ...
          setTimeout(()=>{if(S.uploadingCount===0)hideUploadStatus();},1000);
          resolve(result.url);
        }else{
  ...
  input.onchange=async e=>{
    for(const f of e.target.files){
      ...
      try{
        ...
        S.galleries[gi].images=S.galleries[gi].images.filter(im=>im.id!==tempId);
        renderGalleries();
      }
    }
  };
...
function setupGalleryDrag(){
  document.querySelectorAll('.gallery-group .media-item[draggable="true"]').forEach(el=>{
    el.ondragstart=e=>{e.dataTransfer.setData('g',e.target.dataset.gallery);e.dataTransfer.setData('i',e.target.dataset.index);};
    el.ondragover=e=>e.preventDefault();
    el.ondrop=e=>{
      e.preventDefault();
      const sg=parseInt(e.dataTransfer.getData('g')),si=parseInt(e.dataTransfer.getData('i'));
      const dg=parseInt(e.currentTarget.dataset.gallery),di=parseInt(e.currentTarget.dataset.index);
      if(sg===dg&&si!==di){
        const item=S.galleries[sg].images.splice(si,1)[0];
        S.galleries[sg].images.splice(di,0,item);
        renderGalleries();
      }
    };
  });
...
function addVideo(){
  S.videos.push({id:Date.now(),url:'',videoFile:null,thumbnail:'',thumbnailPreview:'',uploading:false});
  renderVideos();
}
...
function renderAddons(){
  ...
  if(S.addons.length===0){
    c.innerHTML='<p style="text-align:center;color:var(--text-light);padding:2rem">Click button above to add field</p>';
    return;
  }
...
async function save(){
  if(S.uploadingCount>0){alert('⏳ Uploading...');return;}
