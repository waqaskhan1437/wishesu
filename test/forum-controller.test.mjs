...
class Statement {
  ...
  async first() {
    ...
    if (sql.includes("select id from forum_questions where slug is null or trim(slug) = '' limit 1")) {
      return null;
    }

    if (sql.includes("select slug from forum_questions where slug is not null and trim(slug) != '' group by slug having count(*) > 1 limit 1")) {
      const seen = new Set();
