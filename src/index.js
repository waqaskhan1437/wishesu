...
/**
 * Batch upsert multiple meta tags in a single pass instead of N individual
 * regex scans. Each entry: { type, name, content, alwaysReplace }.
 * Tags that already exist are replaced; missing tags are appended before </head>.
 */
...
async function getSeoSettingsObject(env) {
  try {
    // Direct cache access avoids json() → Response → parse overhead per request
    return await getSeoSettingsDirect(env) || {};
  } catch (_) {}
  return {};
}
...
function applyDefaultPageMetadata(html, pageType, seo = {}) {
  ...
  // Use configured og_image or fall back to favicon.svg (always exists)
  const ogImage = String(seo.ogImage || '').trim();
  const siteOrigin = canonical ? (() => { try { return new URL(canonical).origin; } catch (_) { return ''; } })() : '';
...
async function getCachedSettings(env, keys) {
  ...
  for (const key of keys) {
    ...
    const cached = settingsCache.get(kvCacheKey);
    if (cached && (now - cached.ts) < SETTINGS_CACHE_TTL) {
      ...
      try {
        ...
        if (kvData !== null) {
          settingsCache.set(kvCacheKey, { value: kvData.value, ts: now });
...
function ensureGlobalComponentsRuntimeScript(html) {
  if (!html) return html;
  const source = String(html);
  // Skip if already present or no body tag
...
async function applyGlobalComponentsSsr(env, html, pathname) {
  ...
  if (injectedHeader || injectedFooter) {
    // Batch all body data attributes in a single regex replacement
...
// -------------------------------
const SITEMAP_MEMBERSHIP_TTL_MS = 5 * 60 * 1000; // 5 minutes (was 2 min)
 ...
 */
// Per-path SEO result cache (avoids repeated sitemap parsing + visibility rule checks)
const SEO_REQUEST_CACHE_TTL_MS = 3 * 60 * 1000; // 3 minutes
const seoRequestCache = new Map();
...
async function getSeoForRequest(env, req, opts = {}) {
  ...
  // Check per-path cache (skip for product pages since canonical varies)
...
function applySeoToHtml(html, robots, canonical, meta = {}) {
  ...
  // Collect tags to append before </head> in a single replacement
  const headAppend = [];
  ...
  // Flush any accumulated head-append tags in one replace
  ...
  // Batch all meta tag upserts in fewer passes instead of 10 individual regex scans
  const metaTags = [
  ...
  // which runs after this function, so we skip it here to avoid double execution.
  return out;
}
...
async function queryProductsForComponentSsr(env, options = {}) {
  ...
  const cacheKeyStr = requestedIds.length > 0
  ...
  // OPTIMIZED: Run count + product queries in parallel, use LEFT JOIN instead of correlated subqueries
...
const blogQueryCache = new Map();
const BLOG_QUERY_CACHE_TTL = 3 * 60 * 1000;
...
const forumQueryCache = new Map();
const FORUM_QUERY_CACHE_TTL = 3 * 60 * 1000;
...
const reviewQueryCache = new Map();
const REVIEW_QUERY_CACHE_TTL = 3 * 60 * 1000;
      ...
      if ((method === 'GET' || method === 'HEAD') && path.startsWith('/forum/') && path !== '/forum/' && !path.includes('.')) {
        ...
        if (slug && env.DB) {
          try {
            ...
            if (question) {
              const forumSeoPath = question?.slug
          ...
          try {
            ...
            if (defaultPage && defaultPage.content) {
              ...
              try {
                ...
                // Forum archive: inject CollectionPage schema
                ...
                // Product grid: inject CollectionPage schema
              ...
              if (productId && env.DB) {
                ...
                if (product) {
                    ...
                    try {
                      ...
                      // Replace generic sr-only H1 with actual product title
  ...
  async scheduled(event, env, ctx) {
    ...
    try {
      if (env.DB) {
        ...
        // Keep the 5-minute cron cheap. A single trivial query is enough to
        // keep the D1 connection path warm; full schema init and table scans
        // belong to the daily maintenance job.
        ...
        if (isDailyMaintenanceCron) {
          // Daily maintenance can afford a slightly broader health check after
          // schema initialization, but it should still avoid full table scans.
          await Promise.all([
